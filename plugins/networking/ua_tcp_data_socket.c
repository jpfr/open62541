...
//static UA_StatusCode
//UA_ClientConnectionTCP_poll(UA_Client *client, UA_Connection *connection) {
//
...
UA_TCP_DataSocket_ConnectTo(UA_String endpointUrl,
    ...
    retval =
        UA_parseEndpointUrl(&endpointUrl, &hostnameString,
                            &port, &pathString);
    if(retval != UA_STATUSCODE_GOOD || hostnameString.length >= UA_HOSTNAME_MAX_LENGTH) {
        UA_LOG_WARNING(logger, UA_LOGCATEGORY_NETWORK,
                       "Server url is invalid: %*.s", (int)endpointUrl.length, endpointUrl.data);
        return UA_STATUSCODE_BADINTERNALERROR;
    }
    memcpy(hostname, hostnameString.data, hostnameString.length);
    hostname[hostnameString.length] = 0;
