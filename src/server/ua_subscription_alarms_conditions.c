...
acknowledgeMethodCallback(UA_Server *server, const UA_NodeId *sessionId,
    ...
    if(!isNodeInTree(server, &eventType, &AcknowledgeableConditionTypeId,
...
isConditionSourceInMonitoredItem(UA_Server *server, const UA_MonitoredItem *monitoredItem,
    ...
    return isNodeInTree(server, conditionSource, &monitoredItem->monitoredNodeId,
                        UA_REFTYPESET(UA_REFERENCETYPEINDEX_ORGANIZES) |
                        UA_REFTYPESET(UA_REFERENCETYPEINDEX_HASCOMPONENT) |
                        UA_REFTYPESET(UA_REFERENCETYPEINDEX_HASEVENTSOURCE) |
                        UA_REFTYPESET(UA_REFERENCETYPEINDEX_HASNOTIFIER));
}
...
doesHasEventSourceReferenceExist(UA_Server *server, const UA_NodeId nodeToCheck) {
    ...
    for(size_t i = 0; i < node->referencesSize; i++) {
        if((node->references[i].referenceTypeIndex == UA_REFERENCETYPEINDEX_HASEVENTSOURCE ||
            isNodeInTree(server, &server->referenceTypeIds[node->references[i].referenceTypeIndex],
                         &hasEventSourceId, UA_REFTYPESET(UA_REFERENCETYPEINDEX_HASSUBTYPE))) &&
...
setStandardConditionFields(UA_Server *server, const UA_NodeId* condition,
    ...
    UA_NodeId acknowledgeableConditionTypeId =
        UA_NODEID_NUMERIC(0, UA_NS0ID_ACKNOWLEDGEABLECONDITIONTYPE);
    if(!isNodeInTree(server, conditionType, &acknowledgeableConditionTypeId,
                     UA_REFTYPESET(UA_REFERENCETYPEINDEX_HASSUBTYPE)))
        return UA_STATUSCODE_GOOD;
    ...
    if(isNodeInTree(server, conditionType, &alarmConditionTypeId,
                    UA_REFTYPESET(UA_REFERENCETYPEINDEX_HASSUBTYPE))) {
...
setConditionMethodCallbacks(UA_Server *server, const UA_NodeId* condition,
    ...
    UA_StatusCode retval = UA_STATUSCODE_GOOD;
    retval |= UA_Server_setMethodNode_callback(server, methodId[0], disableMethodCallback);
    retval |= UA_Server_setMethodNode_callback(server, methodId[1], enableMethodCallback);
    retval |= UA_Server_setMethodNode_callback(server, methodId[2], addCommentMethodCallback);
    retval |= UA_Server_setMethodNode_callback(server, methodId[3], refreshMethodCallback);
    retval |= UA_Server_setMethodNode_callback(server, methodId[4], refresh2MethodCallback);
    retval |= UA_Server_setMethodNode_callback(server, methodId[5], acknowledgeMethodCallback);
#ifdef CONDITIONOPTIONALFIELDS_SUPPORT
    retval |= UA_Server_setMethodNode_callback(server, methodId[6], confirmMethodCallback);
#endif
...
UA_Server_createCondition(UA_Server *server,
    ...
    if(!isNodeInTree(server, &conditionType, &conditionTypeId,
                     UA_REFTYPESET(UA_REFERENCETYPEINDEX_HASSUBTYPE))) {
        UA_LOG_ERROR(&server->config.logger, UA_LOGCATEGORY_USERLAND,
