...
getNodeType(UA_Server *server, const UA_Node *node) {
    ...
    for(size_t i = 0; i < node->referencesSize; ++i) {
        ...
        const UA_NodeId *typeId = &node->references[i].targetIds[0].nodeId;
        const UA_Node *type =
        ...
        if(type->nodeClass == typeNodeClass)
            return type;
        server->config.nodestore.releaseNode(server->config.nodestore.context, type);
    }
    return NULL;
}
...
UA_Server_editNode(UA_Server *server, UA_Session *session,
    ...
    UA_StatusCode retval = callback(server, session, editNode, data);
