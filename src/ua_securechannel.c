...
UA_SecureChannel_init(UA_SecureChannel *channel) {
    ...
    channel->state = UA_SECURECHANNELSTATE_NEW;
...
UA_SecureChannel_close(UA_SecureChannel *channel) {
    ...
    if(channel->socket) {
        channel->socket->context = NULL;
        channel->socket->close(channel->socket);
        channel->socket = NULL;
    }
...
UA_SecureChannel_sendAsymmetricOPNMessage(UA_SecureChannel *channel,
    ...
    UA_StatusCode retval =
        sock->acquireSendBuffer(sock, channel->config.sendBufferSize, &buf);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
...
sendSymmetricEncodingCallback(void *data, UA_Byte **buf_pos, const UA_Byte **buf_end) {
    ...
    retval = sock->acquireSendBuffer(sock, channel->config.sendBufferSize,
                                     &mc->messageBuffer);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
...
UA_MessageContext_begin(UA_MessageContext *mc, UA_SecureChannel *channel,
    ...
    /* Allocate the message buffer */
    UA_StatusCode retval =
        sock->acquireSendBuffer(sock, channel->config.sendBufferSize,
                                &mc->messageBuffer);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
...
UA_SecureChannel_isMessageComplete(UA_SecureChannel *channel, UA_UInt32 requestId) {
    ...
    TAILQ_FOREACH(message, &channel->messages, pointers) {
        /* Stop at the first incomplete message */
        if(message->requestId == requestId && message->final)
            return true;
    }
    return false;
}
