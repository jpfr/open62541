...
ServerNetworkLayerTCP_listen(UA_ServerNetworkLayer *nl, UA_Server *server,
    ...
    for(UA_UInt16 i = 0; i < layer->serverSocketsSize; i++) {
        ...
        UA_SOCKET newsockfd =
            UA_accept((UA_SOCKET)layer->serverSockets[i],
    ...
    LIST_FOREACH_SAFE(e, &layer->connections, pointers, e_tmp) {
        if(e->connection.state == UA_CONNECTION_OPENING &&
           now > (e->connection.openingDate + (NOHELLOTIMEOUT * UA_DATETIME_MSEC))) {
            UA_LOG_INFO(layer->logger, UA_LOGCATEGORY_NETWORK, "Connection %i | "
                        "Closing (no HEL received)", (int)(e->connection.sockfd));
        ...
        UA_LOG_TRACE(layer->logger, UA_LOGCATEGORY_NETWORK, "Connection %i | "
                     "Activity on the socket", (int)(e->connection.sockfd));
        ...
        if(retval == UA_STATUSCODE_GOOD) {
            ...
            UA_LOG_INFO(layer->logger, UA_LOGCATEGORY_NETWORK,
                        "Connection %i | Closed", (int)(e->connection.sockfd));
            LIST_REMOVE(e, pointers);
            UA_close(e->connection.sockfd);
            UA_Server_removeConnection(server, &e->connection);
        }
...
UA_StatusCode UA_ClientConnectionTCP_poll(UA_Client *client, void *data) {
    ...
    UA_DateTime remaining = UA_DateTime_nowMonotonic() - tcpConnection->connStart;
    if(remaining > (UA_DateTime)(tcpConnection->timeout * UA_DATETIME_MSEC)) {
...
#ifdef _OS9000
        /* OS-9 can't use select for checking write sockets. Therefore, we need
         * to use connect until success or failed */
        UA_UInt32 timeout_usec = (tcpConnection->timeout - timeSinceStart) * 1000;
        int resultsize = 0;
        do {
...
UA_ClientConnectionTCP_init(UA_ConnectionConfig config, const UA_String endpointUrl,
    ...
    if(parse_retval != UA_STATUSCODE_GOOD || hostnameString.length > 511) {
        UA_LOG_WARNING(logger, UA_LOGCATEGORY_NETWORK, "Server url is invalid: %.*s",
...
UA_ClientConnectionTCP(UA_ConnectionConfig config, const UA_String endpointUrl,
    ...
    do {
        /* Get a socket */
        clientsockfd = UA_socket(server->ai_family, server->ai_socktype, server->ai_protocol);
        if(clientsockfd == UA_INVALID_SOCKET) {
