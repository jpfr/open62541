 ...
 * -------------------------------------
 *
 * ReferenceTypes have an alternative represention as an index into a 64-bit
 * bitfields for fast comparison. Every ReferenceType has a different index that
 * is generated when the corresponding ReferenceTypeNode is added. The
 * ReferenceTypeNodes contains a bitfield that represents the ReferenceType and
 * all its subtypes. This speeds up the the Browse services substantially.
 *
 * But this also means that a server can contain at most 64 distinct
 * ReferenceTypes. That should be more than enough even with common
 * Companion Specifications added to the information model. */
...
typedef UA_UInt64 UA_ReferenceTypeSet;

static UA_INLINE UA_Boolean
UA_ReferenceTypeSet_contains(UA_ReferenceTypeSet set, UA_Byte index) {
    return !!(set & (((UA_UInt64)1) << index));
}

static UA_INLINE UA_ReferenceTypeSet
UA_REFTYPESET(UA_Byte index) {
    return ((UA_UInt64)1) << index;
}
