...
(*UA_Socket_DataCallbackFunction)(UA_ByteString *data, UA_Socket *socket);
...
struct UA_Socket {
    /**
     * The socket id. Used by the NetworkManager to map to an internal
     * representation (e.g. file descriptor)
     */
    UA_UInt64 id;
    ...
    /**
     * This callback is called when the socket->open function successfully
     * returns.
     */
    UA_SocketCallbackFunction openCallback;

    /**
     * This callback is called when the socket is freed with the socket->free
     * function.
     */
    UA_SocketCallbackFunction freeCallback;

    /**
     * The dataCallback is called by the socket once it has sufficient data that
     * it can pass on to be processed. The data ByteString will be reused or
     * deallocated once the callback returns. If the data is needed beyond the
     * call, it needs to be copied, otherwise it will be lost.
     */
    ...
    /**
     * Starts/Opens the socket for operation. This step is separate from the
     * initialization so the sockets can be configured without starting to
     * listen already.
     *
    ...
    /**
     * Checks if the socket can be deleted because it has been closed by the
     * local application, or if it was closed remotely.
     *
     * \param socket the socket to perform the operation on.
    ...
    /**
     * The application pointer points to the application the socket is
     * associated with. (e.g server/client)
     */
    void *application;

    /**
     * The context can be a pointer to any kind of data that is needed for
     * example for the data and free callbacks.
     */
    void *context;
};
...
struct UA_SocketConfig {
    ...
    /**
     * This function is called by the server to create all configured listener
     * sockets. The delayed configuration makes sure, that initialization is
     * done during server startup. Also, only the server will have ownership of
     * the Sockets.
     *
     * \param parameters
     * \param creationCallback The socketCallback is called once for each socket that is created.
     */
    UA_StatusCode (*createSocket)(const UA_SocketConfig *parameters,
...
struct UA_NetworkManager {
    ...
    UA_StatusCode (*createSocket)(UA_NetworkManager *networkManager,
                                  UA_SocketConfig *const socketParameters,
                                  const UA_SocketCallbackFunction creationCallback);
    ...
    UA_StatusCode (*processSocket)(UA_NetworkManager *networkManager,
                                   UA_UInt32 timeout, UA_Socket *sock);
